...
#define mod_eye  7 // Eyes:  0-mk3; 1-ellipse; 2-classic; 3-triangular; 5-strange; 4-semicircular;                        ║12 options ║
...
//                                                                                                                        ║
//                                                         Timings                                                        ║
int owo_tm = 600; // Astonishment duration in milliseconds   6000                                                            ║
int zloy_tm = 8000;// Anger duration in milliseconds            8000                                                          ║
int tm_dt = 1200; // Duration of death in milliseconds           12000                                                        ║
int hrt_spd = 400; // Heart animation speed, the smaller the number, the faster the animation.                            ║
...
Max72xxPanel matrix = Max72xxPanel(9, 1, 16); //matrix connection, only I allowed to touch 
    ...
    if (alt_owo != 0){
          for (int y = 0; y < 40; y++){
            for (int x = 8; x >= 0; x--){
              ...
              matrix.drawPixel(x, 79-y, o[y] & (1 << x));
    }
    }
    matrix.write();
    }
        else{
      for (int y = 40*rot; y < 40+40*rot; y++){
        for (int x = 8; x >= 0; x--){
          matrix.drawPixel(x, y-40*rot, jaws[y] & (1 << x));
          matrix.drawPixel(x, 79+40*rot-y, jaws[y] & (1 << x));
      }
    }
    matrix.write();
    }
    ...
    if (alt_ang != 0){
      for (int y = 0; y < 40; y++){
        for (int x = 8; x >= 0; x--){
          ...
          matrix.drawPixel(x, 79-y, z[y] & (1 << x));
      }
    }
    matrix.write();
    }
